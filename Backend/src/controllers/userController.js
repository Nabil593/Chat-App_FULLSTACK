...
export async function sendFriendRequest(req, res) {
    try {
        ...
        if (myId === recipientId) {
            return res.status(400).json({message: "You can't send friend request tp yourself"});
        }
        ...
        if (recipient.friends.includes(myId)) {
            return res.status(400).json({message: "You sre already friends with this user"});
        }
...
export async function getFriendRequest(req, res) {
    try {
        ...
        const acceptedReqs = await FriendRequest.find({
            sender: req.user.id,
            status: "accepted",
        }).populate("recipient", "fullname profilePic nativeLanguage learningLanguage"); // fix typo
